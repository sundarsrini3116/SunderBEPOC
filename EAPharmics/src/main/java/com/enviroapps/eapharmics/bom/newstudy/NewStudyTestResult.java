package com.enviroapps.eapharmics.bom.newstudy;

import java.io.Serializable;
import java.util.Date;

import com.enviroapps.eapharmics.bom.product.ProductTest;

public class NewStudyTestResult {

	private static final long serialVersionUID = 1L;

	private Long prdStudyTestResultsId;
	private Long prdStudyScheduleId;
	private Long productTestId;
	private String analyst;
	private String reference;
	private Date resultDate;
	private String resultStatus;
	private String result1;
	private String result2;
	private String result3;
	private String result4;
	private String result5;
	private String result6;
	private String result7;
	private String result8;
	private String result9;
	private String result10;
	private String result11;
	private String result12;
	private String result13;
	private String result14;
	private String result15;
	private String result16;
	private String result17;
	private String result18;
	private String result19;
	private String result20;
	private String result21;
	private String result22;
	private String result23;
	private String result24;
	private String result25;
	private String result26;
	private String result27;
	private String result28;
	private String result29;
	private String result30;
	private Date insertDate;
	private Long insertUser;
	private Date updateDate;
	private Long updateUser;
	private Long auditId;
	private String comments;
	private Long prdStudyBatchId;
	private String studyId;
	private boolean testIntervalUsed;
	private Double fillUnitsUsed;
	private String usedBy;
	private Date usedDate;
	private ProductTest productTest;
	private String resultAverage;

	/* (non-Javadoc)
	 * @see com.enviroapps.das.persistence.AbstractPersistableObject#getPK()
	 */
	public Serializable getPK() {
		return prdStudyTestResultsId;
	}
	/* (non-Javadoc)
	 * @see com.enviroapps.das.persistence.AbstractPersistableObject#loadPK(java.lang.Object[])
	 */
	public void loadPK(Object[] pks) {
		prdStudyTestResultsId = (Long)pks[0];
	}
	/**
	 * @return the prdStudyTestResultsId
	 */
	public Long getPrdStudyTestResultsId() {
		return prdStudyTestResultsId;
	}
	/**
	 * @param prdStudyTestResultsId the prdStudyTestResultsId to set
	 */
	public void setPrdStudyTestResultsId(Long prdStudyTestResultsId) {
		this.prdStudyTestResultsId = prdStudyTestResultsId;
	}
	/**
	 * @return the prdStudyScheduleId
	 */
	public Long getPrdStudyScheduleId() {
		return prdStudyScheduleId;
	}
	/**
	 * @param prdStudyScheduleId the prdStudyScheduleId to set
	 */
	public void setPrdStudyScheduleId(Long prdStudyScheduleId) {
		this.prdStudyScheduleId = prdStudyScheduleId;
	}
	/**
	 * @return the productTestId
	 */
	public Long getProductTestId() {
		return productTestId;
	}
	/**
	 * @param productTestId the productTestId to set
	 */
	public void setProductTestId(Long productTestId) {
		this.productTestId = productTestId;
	}
	/**
	 * @return the analyst
	 */
	public String getAnalyst() {
		return analyst;
	}
	/**
	 * @param analyst the analyst to set
	 */
	public void setAnalyst(String analyst) {
		this.analyst = analyst;
	}
	/**
	 * @return the reference
	 */
	public String getReference() {
		return reference;
	}
	/**
	 * @param reference the reference to set
	 */
	public void setReference(String reference) {
		this.reference = reference;
	}
	/**
	 * @return the resultDate
	 */
	public Date getResultDate() {
		return resultDate;
	}
	/**
	 * @param resultDate the resultDate to set
	 */
	public void setResultDate(Date resultDate) {
		this.resultDate = resultDate;
	}
	/**
	 * @return the resultStatus
	 */
	public String getResultStatus() {
		return resultStatus;
	}
	/**
	 * @param resultStatus the resultStatus to set
	 */
	public void setResultStatus(String resultStatus) {
		this.resultStatus = resultStatus;
	}
	/**
	 * @return the result1
	 */
	public String getResult1() {
		return result1;
	}
	/**
	 * @param result1 the result1 to set
	 */
	public void setResult1(String result1) {
		this.result1 = result1;
	}
	/**
	 * @return the result2
	 */
	public String getResult2() {
		return result2;
	}
	/**
	 * @param result2 the result2 to set
	 */
	public void setResult2(String result2) {
		this.result2 = result2;
	}
	/**
	 * @return the result3
	 */
	public String getResult3() {
		return result3;
	}
	/**
	 * @param result3 the result3 to set
	 */
	public void setResult3(String result3) {
		this.result3 = result3;
	}
	/**
	 * @return the result4
	 */
	public String getResult4() {
		return result4;
	}
	/**
	 * @param result4 the result4 to set
	 */
	public void setResult4(String result4) {
		this.result4 = result4;
	}
	/**
	 * @return the result5
	 */
	public String getResult5() {
		return result5;
	}
	/**
	 * @param result5 the result5 to set
	 */
	public void setResult5(String result5) {
		this.result5 = result5;
	}
	/**
	 * @return the result6
	 */
	public String getResult6() {
		return result6;
	}
	/**
	 * @param result6 the result6 to set
	 */
	public void setResult6(String result6) {
		this.result6 = result6;
	}
	/**
	 * @return the result7
	 */
	public String getResult7() {
		return result7;
	}
	/**
	 * @param result7 the result7 to set
	 */
	public void setResult7(String result7) {
		this.result7 = result7;
	}
	/**
	 * @return the result8
	 */
	public String getResult8() {
		return result8;
	}
	/**
	 * @param result8 the result8 to set
	 */
	public void setResult8(String result8) {
		this.result8 = result8;
	}
	/**
	 * @return the result9
	 */
	public String getResult9() {
		return result9;
	}
	/**
	 * @param result9 the result9 to set
	 */
	public void setResult9(String result9) {
		this.result9 = result9;
	}
	/**
	 * @return the result10
	 */
	public String getResult10() {
		return result10;
	}
	/**
	 * @param result10 the result10 to set
	 */
	public void setResult10(String result10) {
		this.result10 = result10;
	}
	/**
	 * @return the result11
	 */
	public String getResult11() {
		return result11;
	}
	/**
	 * @param result11 the result11 to set
	 */
	public void setResult11(String result11) {
		this.result11 = result11;
	}
	/**
	 * @return the result12
	 */
	public String getResult12() {
		return result12;
	}
	/**
	 * @param result12 the result12 to set
	 */
	public void setResult12(String result12) {
		this.result12 = result12;
	}
	/**
	 * @return the result13
	 */
	public String getResult13() {
		return result13;
	}
	/**
	 * @param result13 the result13 to set
	 */
	public void setResult13(String result13) {
		this.result13 = result13;
	}
	/**
	 * @return the result14
	 */
	public String getResult14() {
		return result14;
	}
	/**
	 * @param result14 the result14 to set
	 */
	public void setResult14(String result14) {
		this.result14 = result14;
	}
	/**
	 * @return the result15
	 */
	public String getResult15() {
		return result15;
	}
	/**
	 * @param result15 the result15 to set
	 */
	public void setResult15(String result15) {
		this.result15 = result15;
	}
	/**
	 * @return the result16
	 */
	public String getResult16() {
		return result16;
	}
	/**
	 * @param result16 the result16 to set
	 */
	public void setResult16(String result16) {
		this.result16 = result16;
	}
	/**
	 * @return the result17
	 */
	public String getResult17() {
		return result17;
	}
	/**
	 * @param result17 the result17 to set
	 */
	public void setResult17(String result17) {
		this.result17 = result17;
	}
	/**
	 * @return the result18
	 */
	public String getResult18() {
		return result18;
	}
	/**
	 * @param result18 the result18 to set
	 */
	public void setResult18(String result18) {
		this.result18 = result18;
	}
	/**
	 * @return the result19
	 */
	public String getResult19() {
		return result19;
	}
	/**
	 * @param result19 the result19 to set
	 */
	public void setResult19(String result19) {
		this.result19 = result19;
	}
	/**
	 * @return the result20
	 */
	public String getResult20() {
		return result20;
	}
	/**
	 * @param result20 the result20 to set
	 */
	public void setResult20(String result20) {
		this.result20 = result20;
	}
	/**
	 * @return the result21
	 */
	public String getResult21() {
		return result21;
	}
	/**
	 * @param result21 the result21 to set
	 */
	public void setResult21(String result21) {
		this.result21 = result21;
	}
	/**
	 * @return the result22
	 */
	public String getResult22() {
		return result22;
	}
	/**
	 * @param result22 the result22 to set
	 */
	public void setResult22(String result22) {
		this.result22 = result22;
	}
	/**
	 * @return the result23
	 */
	public String getResult23() {
		return result23;
	}
	/**
	 * @param result23 the result23 to set
	 */
	public void setResult23(String result23) {
		this.result23 = result23;
	}
	/**
	 * @return the result24
	 */
	public String getResult24() {
		return result24;
	}
	/**
	 * @param result24 the result24 to set
	 */
	public void setResult24(String result24) {
		this.result24 = result24;
	}
	/**
	 * @return the result25
	 */
	public String getResult25() {
		return result25;
	}
	/**
	 * @param result25 the result25 to set
	 */
	public void setResult25(String result25) {
		this.result25 = result25;
	}
	/**
	 * @return the result26
	 */
	public String getResult26() {
		return result26;
	}
	/**
	 * @param result26 the result26 to set
	 */
	public void setResult26(String result26) {
		this.result26 = result26;
	}
	/**
	 * @return the result27
	 */
	public String getResult27() {
		return result27;
	}
	/**
	 * @param result27 the result27 to set
	 */
	public void setResult27(String result27) {
		this.result27 = result27;
	}
	/**
	 * @return the result28
	 */
	public String getResult28() {
		return result28;
	}
	/**
	 * @param result28 the result28 to set
	 */
	public void setResult28(String result28) {
		this.result28 = result28;
	}
	/**
	 * @return the result29
	 */
	public String getResult29() {
		return result29;
	}
	/**
	 * @param result29 the result29 to set
	 */
	public void setResult29(String result29) {
		this.result29 = result29;
	}
	/**
	 * @return the result30
	 */
	public String getResult30() {
		return result30;
	}
	/**
	 * @param result30 the result30 to set
	 */
	public void setResult30(String result30) {
		this.result30 = result30;
	}
	/**
	 * @return the insertDate
	 */
	public Date getInsertDate() {
		return insertDate;
	}
	/**
	 * @param insertDate the insertDate to set
	 */
	public void setInsertDate(Date insertDate) {
		this.insertDate = insertDate;
	}
	/**
	 * @return the insertUser
	 */
	public Long getInsertUser() {
		return insertUser;
	}
	/**
	 * @param insertUser the insertUser to set
	 */
	public void setInsertUser(Long insertUser) {
		this.insertUser = insertUser;
	}
	/**
	 * @return the updateDate
	 */
	public Date getUpdateDate() {
		return updateDate;
	}
	/**
	 * @param updateDate the updateDate to set
	 */
	public void setUpdateDate(Date updateDate) {
		this.updateDate = updateDate;
	}
	/**
	 * @return the updateUser
	 */
	public Long getUpdateUser() {
		return updateUser;
	}
	/**
	 * @param updateUser the updateUser to set
	 */
	public void setUpdateUser(Long updateUser) {
		this.updateUser = updateUser;
	}
	/**
	 * @return the auditId
	 */
	public Long getAuditId() {
		return auditId;
	}
	/**
	 * @param auditId the auditId to set
	 */
	public void setAuditId(Long auditId) {
		this.auditId = auditId;
	}
	/**
	 * @return the comments
	 */
	public String getComments() {
		return comments;
	}
	/**
	 * @param comments the comments to set
	 */
	public void setComments(String comments) {
		this.comments = comments;
	}
	/**
	 * @return the prdStudyBatchId
	 */
	public Long getPrdStudyBatchId() {
		return prdStudyBatchId;
	}
	/**
	 * @param prdStudyBatchId the prdStudyBatchId to set
	 */
	public void setPrdStudyBatchId(Long prdStudyBatchId) {
		this.prdStudyBatchId = prdStudyBatchId;
	}
	/**
	 * @return the studyId
	 */
	public String getStudyId() {
		return studyId;
	}
	/**
	 * @param studyId the studyId to set
	 */
	public void setStudyId(String studyId) {
		this.studyId = studyId;
	}
	/**
	 * @return the testIntervalUsed
	 */
	public boolean getTestIntervalUsed() {
		return testIntervalUsed;
	}
	/**
	 * @param testIntervalUsed the testIntervalUsed to set
	 */
	public void setTestIntervalUsed(boolean testIntervalUsed) {
		this.testIntervalUsed = testIntervalUsed;
	}
	/**
	 * @return the productTest
	 */
	public ProductTest getProductTest() {
		return productTest;
	}
	/**
	 * @param productTest the productTest to set
	 */
	public void setProductTest(ProductTest productTest) {
		this.productTest = productTest;
	}
	/**
	 * @return the fillUnitsUsed
	 */
	public Double getFillUnitsUsed() {
		return fillUnitsUsed;
	}
	/**
	 * @param fillUnitsUsed the fillUnitsUsed to set
	 */
	public void setFillUnitsUsed(Double fillUnitsUsed) {
		this.fillUnitsUsed = fillUnitsUsed;
	}
	/**
	 * @return the usedBy
	 */
	public String getUsedBy() {
		return usedBy;
	}
	/**
	 * @param usedBy the usedBy to set
	 */
	public void setUsedBy(String usedBy) {
		this.usedBy = usedBy;
	}
	/**
	 * @return the usedDate
	 */
	public Date getUsedDate() {
		return usedDate;
	}
	/**
	 * @param usedDate the usedDate to set
	 */
	public void setUsedDate(Date usedDate) {
		this.usedDate = usedDate;
	}
	/**
	 * @return the resultAverage
	 */
	public String getResultAverage() {
		return resultAverage;
	}
	/**
	 * @param resultAverage the resultAverage to set
	 */
	public void setResultAverage(String resultAverage) {
		this.resultAverage = resultAverage;
	}
}
